...
fn set_mi_ma((min, max): (i32, i32), x: &i32) -> (i32, i32) {
    if x < &min {
        (*x, max)
    } else if x > &max {
        (min, *x)
...
fn diff_min_max(x: &Vec<i32>) -> i32 {
    let (mi, ma) = x
        .iter()
        .fold((x[0], x[0]), set_mi_ma);
    ma - mi
}

fn part1(x: &str) -> i32 {
    int_matrix_from_str(x)
        .iter()
        .map(diff_min_max)
        .sum()
}

fn get_zero_rest_division_partner(x: i32, nrs: &[i32]) -> Option<&i32> {
    nrs.iter().find(|&&y| x % y == 0 || y % x == 0)
}

fn get_dividable_pair(x: &Vec<i32>) -> (i32, i32) {
    match x
        .iter()
        .enumerate()
        .find_map(|(i, &val)| {
            match get_zero_rest_division_partner(val, &x[i+1..]) {
                Some(&x) => Some((val, x)),
                None => None
            }
        }) {
...
fn divide_even_pairs(x: &Vec<i32>) -> i32 {
    let (partner1, partner2) = get_dividable_pair(x);
    if (partner1, partner2) == (0, 0) {
        0
    }
